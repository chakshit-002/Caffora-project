...
export default function CreateProduct() {
    ...
    const onCreateProductHandler = (data) => {
        ...
        if (uploadMethod === 'file') {
            ...
            if (!files || files.length < 2) {
                setError("image_files", { type: "manual", message: "Please upload at least 2 images." });
                return;
            }
            ...
            if (validUrls.length < 2) {
                setError("image_urls_error", { type: "manual", message: "Please enter at least 2 image URLs." });
                return;
            }
    ...
    const handleTabChange = (method) => {
        ...
        reset({ // Reset file input value specifically
            image_files: null
        }); 
    };
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4 md:p-8 flex items-center justify-center relative z-20">
            <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl w-full max-w-2xl">
                ...
                <form onSubmit={handleSubmit(onCreateProductHandler)} className="space-y-6">
                            ...
                            <>
                                <input
                                    ...
                                    {...register("image_files", { 
                                        ...
                                        validate: {
                                            minFiles: (value) => value && value.length == 2 || "Please upload 2 images only"
                                        }
                                    })}
                                    onChange={(e) => {
                        ...
                        {uploadMethod === 'url' && (
                            <>
                                ...
                                <div className="flex justify-between items-center mt-4">
                                    <p className='text-xs text-gray-500'>Current URLs: {imageUrls.filter(url => url.trim() !== '').length} (2 only)</p>
                    ...
                    <button
                        type="submit"
                        className="w-full bg-[#e57c23] text-white font-bold py-3 rounded-lg hover:bg-[#cf6d1f] transition duration-300 shadow-md"
                    >
                        Create Product
                    </button>
                </form>
