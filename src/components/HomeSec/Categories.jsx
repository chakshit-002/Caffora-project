...
//   return (
//     <div ref={containerRef} className="relative min-h-[300vh] bg-[#FCF7E6]">
...
export default function Categories() {
  ...
  const motion = useRef({ x: 0, y: 0, rotY: 0, tiltZ: 0, scale: 1 });
  const scrollAnim = useRef(null);
  const idleTl = useRef(null);

  const sections = useMemo(() => [1, 2, 3, 4, 5], []);
  ...
  if (imageRefs.current.length !== sections.length) {
    imageRefs.current = Array(sections.length)
      .fill()
      .map((_, i) => imageRefs.current[i] || React.createRef());
  }

  useEffect(() => {
    const observers = imageRefs.current.map((imgRef, idx) => {
      if (!imgRef.current) return null;
      const observer = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting && !imgRef.current.src) {
            imgRef.current.src = `https://images.pexels.com/photos/437716/pexels-photo-437716.jpeg?text=Category+${idx + 1}`;
          }
        },
        { threshold: 0.1 }
      );
  ...
  useEffect(() => {
    ...
    const spinLoop = () => {
      if (spinActive && !isDragging) motion.current.rotY += 0.01;
      spinReq = requestAnimationFrame(spinLoop);
    };
    spinLoop();
    ...
    const handleMove = (e) => {
      ...
      const dy = e.clientY - startY;
      gsap.to(motion.current, {
        rotY: currentRotY + dx * 0.005,
        tiltZ: currentTiltZ + dy * 0.005,
        duration: 0.3,
        ease: "power2.out",
      });
    };
    ...
    const handleUp = () => {
      ...
      resumeTimeout = setTimeout(() => (spinActive = true), 2000);
    };
  ...
  const showFloatingCup = !isDocked;
  const showDockedCup = isDocked;
            ...
            <div className="w-full md:w-1/2">
              {i === 5 ? (
                ...
                <img
                  ...
                  alt={`Category ${i}`}
                  className="rounded-xl w-full object-cover shadow-md"
                  // src will be set lazily by IntersectionObserver
                />
              )}
            </div>
