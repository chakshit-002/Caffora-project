...
//         bg-gradient-to-r from-[brown] via-[orange] to-[red]
...
//       style={{
//         backgroundImage:
//           "linear-gradient(110deg,brown 20%,orange 50%,red 80%)",
...
const BackButton = () => {
  ...
  const handleMouseEnter = useCallback(() => {
    if (!btnRef.current || !arrowRef.current || !sparkleRef.current) return;
  ...
  const buttonStyle = useMemo(
    () => ({
      backgroundImage: "linear-gradient(110deg,brown 20%,orange 50%,red 80%)",
      backgroundSize: "200% auto",
      backgroundPosition: "200% center",
      boxShadow: "0 4px 16px rgba(76,110,245,0.07)",
      perspective: "220px",
  ...
  // Only render when needed (conditional rendering can be done in parent)
  return (
    <button
      onClick={BackHandler}
      ref={btnRef}
    ...
    >
      ...
      <span
        ...
        className="absolute left-0 top-1/2 -translate-y-1/2 w-7 h-7 pointer-events-none"
        aria-hidden="true"
        style={{ filter: "drop-shadow(0 0 10px #fffcae)", opacity: 0 }}
      >
        <svg width="24" height="24" fill="none">
          <g>
            <circle cx="12" cy="12" r="4.4" fill="#fff" fillOpacity="0.82" />
